...
void ugr::risk::RiskMap::addPointStrikeMap(const ugr::gridmap::Index& index)
{
    ...
    if (shelterFactorSum > 0.0)
        // This is set for the index of the LoC because that is the convention that has been used to calculate the building impact probs
        // Setting this for the ground location would result in converting back and forth between ground impact location and LoC with no need to
        at("Shelter Factor", index) = (shelterFactorSum / buildingImpactProbs.size()) * 0.9;
        ...
        for (int i = 0; i < nSamples; ++i)
        {
            ...
            for (const auto& trackIndex : groundTrackIndices)
            {
                ...
                if (altAtPoint <= buildingHeightAtPoint)
                {
                    ...
                    // Cannot hit 2 buildings in a single descent so break out
                    break;
                }
            }
