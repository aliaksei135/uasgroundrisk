...
AircraftDescentModel::AircraftDescentModel(double mass, double width,
                                           double length, double cruiseSpeed,
...
ugr::risk::AircraftDescentModel::glideImpact(double altitude) const {
...
ugr::risk::ImpactDataStruct
ugr::risk::AircraftDescentModel::ballisticImpact(double altitude, double velX,
                                                 double velY) const {
  ...
  auto x3 = vixC * exp(-1. / 2 * log(1 - pow(viyC, 2) / pow(gamma, 2))) *
            gamma / GRAVITY_ACCEL *
...
std::vector<ImpactDataStruct> ugr::risk::AircraftDescentModel::ballisticImpact(
    std::vector<double> altitude, std::vector<double> velX,
