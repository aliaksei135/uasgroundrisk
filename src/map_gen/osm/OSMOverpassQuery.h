...
public:
  ...
  OSMTag(const std::string &key) : key(key) {}
  OSMTag(const std::string &key, const std::string &value)
      : key(key), value(value) {}
...
public:
   ...
   * applied to result
   */
  template <typename... THandlers> void makeQuery(THandlers &&...handlers) {
    rawResponse();
...
private:
  const char *responseFilepath = strcat(std::tmpnam(NULL), ".xml");
